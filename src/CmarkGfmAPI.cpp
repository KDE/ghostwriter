...
CmarkGfmAPI* CmarkGfmAPI::instance = NULL;

CmarkGfmAPI* CmarkGfmAPI::getInstance() 
{
	if (NULL == instance)
	{
		instance = new CmarkGfmAPI();
	}
...
MarkdownAST* CmarkGfmAPI::parse(const QString& text, const bool smartTypographyEnabled)
{
    ...
    cmark_parser_attach_syntax_extension(parser, tagfilterExt);
...
{	
	...
	tableExt = cmark_find_syntax_extension("table");
	strikethroughExt = cmark_find_syntax_extension("strikethrough");
	autolinkExt = cmark_find_syntax_extension("autolink");
	tagfilterExt = cmark_find_syntax_extension("tagfilter");
	tasklistExt = cmark_find_syntax_extension("tasklist");
}
